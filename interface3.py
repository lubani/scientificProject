...
class App:
    ...
    def set_gold_standard_temp_array(self, selected_solution):
        # Assuming you have a dictionary mapping solution types to their respective temperature arrays
    ...
    def calcAll(self):
        ...
        # Adjust dimensions of x to align with y_T, y_B
        self.x = np.squeeze(self.x)
        # print("Debug: Shape of x after squeezing:", self.x.shape)

        # Prepare grid arrays for x and t
        self.x_arr2 = self.x_grid  # 1D x array
        self.t_arr2 = self.t_grid  # 1D t array
        # print("Debug: Shape of x:", self.x.shape)

        # Generate meshgrid for temperature and boundary conditions
        X, T = np.meshgrid(self.x_arr2, self.t_arr2, indexing='ij')
        x_temp = X.ravel()  # Flatten for PINN
        x_boundary_2D = np.meshgrid(self.x_arr2, self.t_arr2, indexing='ij')[0]  # 2D for traditional methods

        # Compute flat mask arrays for temperature and boundary conditions
        nt = len(self.t_arr2)
        flat_mask_array_T, flat_mask_array_B = compute_mask_arrays(x_temp, x_boundary_2D.ravel(), self.L, nt)
        ...
        self.mask_array_T = flat_mask_array_T.reshape(len(self.x_arr2), nt)
        self.mask_array_B = flat_mask_array_B.reshape(len(self.x_arr2), nt)

        # Store initial and boundary conditions for PINN
        self.initial_data_T = (self.x, self.y_T)  # Adjust as needed
        self.initial_data_B = (self.x_boundary, self.y_B)  # Adjust as needed
        ...
        self.T_arr_numerical, self.temp_mask_array_numerical, self.bound_mask_array_numerical = \
            self.pcm.calcTemperature3(self.x_arr2, self.t_max, self.pcm, phase_mask_array=self.mask_array_T,
        ...
        self.T_arr_analytical, self.temp_mask_array_analytical, self.bound_mask_array_analytical = \
            self.pcm.explicitSol(self.x_arr2, self.t_arr2, self.pcm, phase_mask_array=self.mask_array_T,
        ...
        self.T_arr_implicit, self.H_arr_final, self.t_arr4, self.temp_mask_array_implicit, self.bound_mask_array_implicit = \
            self.pcm.implicitSol(self.x_arr2, self.t_max, self.pcm, phase_mask_array=self.mask_array_T,
                                 boundary_mask_array=self.mask_array_B)
        ...
        # print(f"Debug: self.x_arr2 length = {len(self.x_arr2)}")
        # print(f"Debug: self.t_arr2 length = {len(self.t_arr2)}")
        ...
        # self.mask_array = compute_mask_array(self.x_arr2, self.L)
        self.update_solution_type()
        self.update_plots()
    ...
    def prepare_PINN_model_and_train(self):
        ...
        # Calculate boundary indices based on your provided method
        ...
        self.batch_size = len(self.x_arr2) * len(self.t_arr2) // 2
        ...
        # Optionally, you could call your plotting function here to show results immediately after training
    ...
    def show_PINN_plots(self, show_gold_standard=True, custom_cmap='coolwarm'):
        ...
        x_dim, t_dim = len(self.x_arr2), len(self.t_arr2)
        ...
        # Reshape Boundary_pred to match the dimensions of x_arr2 and t_arr2
        ...
        ax4.plot(self.x_arr2, Boundary_last_time_step, label='Predicted Boundary at Last Time Step')
